...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    temp_list=[PAC,GHOST0,GHOST1,OBS0,OBS1]
    variables.extend(temp_list) # append the variables to the variables list
    edges_tempList = [(GHOST0,OBS0),(PAC,OBS0),(PAC,OBS1),(GHOST1,OBS1)]
    edges.extend(edges_tempList) # append the edges to the edges list
    ...
    for i in range(0,X_RANGE*Y_RANGE):
        for j in range(0,X_RANGE*Y_RANGE):
            dist=abs(manhattanDistance(variableDomainsDict[PAC][i],variableDomainsDict[GHOST0][j])+MAX_NOISE)
            if dist not in variableDomainsDict[OBS0]:
                variableDomainsDict[OBS0].append(dist)
            dist=abs(manhattanDistance(variableDomainsDict[PAC][i],variableDomainsDict[GHOST0][j])-MAX_NOISE)
            if dist not in variableDomainsDict[OBS0]:
                variableDomainsDict[OBS0].append(dist)

            dist=abs(manhattanDistance(variableDomainsDict[PAC][i],variableDomainsDict[GHOST1][j])+MAX_NOISE)
            if dist not in variableDomainsDict[OBS1]:
                variableDomainsDict[OBS1].append(dist)
            dist=abs(manhattanDistance(variableDomainsDict[PAC][i],variableDomainsDict[GHOST1][j])-MAX_NOISE)
            if dist not in variableDomainsDict[OBS1]:
                variableDomainsDict[OBS1].append(dist)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for hiddenVariable in eliminationOrder: # for all the hidden variables in the elimination order
            # join factors which contain the hidden variable with the hidden variable
            factors, joinedFactors= joinFactorsByVariable(factors,hiddenVariable) 
            # If factor has only one unconditional variable, we can not eliminate that variable
            if(len(joinedFactors.unconditionedVariables()) > 1): 
                eliminateFactor=eliminate(joinedFactors, hiddenVariable)
                factors.append(eliminateFactor)
        NormalizedJoinedFactor=normalize(joinFactors(factors)) # Normalize the joined factor
        return NormalizedJoinedFactor
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
